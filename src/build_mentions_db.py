...
def _process_entities(freebase_json, links_db, pages_db, mentions_conn, commit_frequency, limit_entities):
    ...
    init_args = ((freebase_data, links_db, pages_db),)
    with Pool(4, _init_worker, init_args) as pool:
        for entity_count, mentions in enumerate(pool.imap(_process_entity, freebase_data)):
            insert_mentions(mentions_conn, mentions)
            mentions_conn.commit()
            log('{}'.format(entity_count))
...
def _init_worker(args):
    ...
    freebase_data, links_db, pages_db = args

    nlp = English()
    nlp.vocab.lex_attr_getters = {}


def _process_entity(mid):
    """
...
if __name__ == '__main__':
    _process_entities(
        ...
        sqlite3.connect('data/mentions-dev'),
        None,
        None
    )
